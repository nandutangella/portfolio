...
const Portfolio = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjects = async () => {
      try {
        ...
        // Debug: Log the first project's toolsUsed to confirm it's being received
        if (response.data.length > 0) {
          console.log('First project toolsUsed:', response.data[0].toolsUsed);
        }
  ...
  const loadFullImage = (index) => {
    if (!modalProject || !modalProject.images || !modalProject.images[index]) return;
    const image = modalProject.images[index];
    if (!imageStatuses[image.fullUrl]) {
      ...
      >
        <div className="container mx-auto px-4 text-center">
          <h1
            ...
            style={{
              fontWeight: '200',
              textShadow: '0 8px 24px rgba(90, 120, 150, 0.9)',
              color: 'white',
            }}
          >
            Nandu Tangella
      ...
      <section id="portfolio" className="py-6 flex-grow">
        <div className="container mx-auto px-4">
          {loading ? (
              ...
              {[...Array(6)].map((_, index) => (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="h-4 w-1/2 mt-2 skeleton"></div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <p className="text-center text-gray-600">No recent work found. Projects data: {JSON.stringify(projects)}</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {projects.map((project, index) => {
                const renderTime = performance.now();
                // Debug: Log each project's toolsUsed before rendering
                console.log(`Project ${project.title} toolsUsed:`, project.toolsUsed);
                return (
                  <div
                    key={index}
                  ...
                  >
                    {project.thumbnail ? (
                      <div className="relative overflow-hidden">
                        <img
                          src={project.thumbnail.placeholderUrl}
        ...
        >
          ...
          >
            ...
            <div className="absolute top-16 bottom-16 left-0 right-0 overflow-y-auto flex items-center justify-center pt-16 group">
              {modalProject.images && modalProject.images[currentImageIndex] ? (
